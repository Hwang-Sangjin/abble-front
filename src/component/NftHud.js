...
const MaterialContainer = ({args, color, activeTexture, ...props}) => {
    const activeTextureIndex = Object.keys(nftModels).indexOf(activeTexture);
    return(
        <mesh {...props}>
            {Object.keys(nftModels).map((key, index) => {
                return (
                <Material
                    key={key}
                    count={6}
                    isActive={activeTextureIndex === index}
                    texture={nftModels[key]}
                    args={[0.2, 0.2, 0.05]}
...
export const NftHud = ({position}) => {
    ...
    const [activeNft] = useStore((state) => [state.nft]); // Store 이걸 DB로 바꾸면 됨. Store가 개인이 가지고 있는 NFT와 게임 아이템임. 우리가 들고있다가 넘겨주는 식으로 주면 될듯. state로 관리.
    useFrame(() => {
    ...
    }, [setHudVisible, activeNft]);
    return (
        hudVisible && (
            <group position={hudState.position} rotation={hudState.rotation}>
                <group position={position}>
                    <MaterialContainer
                        args={[1.3, 0.3, 0.01]} // 동적 사이즈 구현해야함. 오브젝트 키 렝스로 받아오면 얼추 나옴.
                        color="#222"
                        activeTexture={activeNft}
